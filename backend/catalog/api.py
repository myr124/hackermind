...
def projects(offset: int = Query(0, ge=0), limit: int = Query(24, ge=1, le=100), space_id: int | None = Query(None, ge=1)):
    ...
    with connect() as conn:
        ...
        rows = conn.execute(REACH + """SELECT p.* FROM projects p
            ...
            AND (%s::bigint IS NULL OR EXISTS (SELECT 1 FROM project_terms pt JOIN reach r ON r.descendant=pt.term_id WHERE pt.project_id=p.id AND r.ancestor=%s))
