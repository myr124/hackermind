...
const state={variant:names[params.get('variant')]?params.get('variant'):'A',selected:null,query:'',tech:new Set(),saved:new Set(),mode:'explore',older:false,expanded:false,theme:'dark',mobile:'graph',zoom:1,panX:0,panY:0};
...
function label(id){return allNodes.find(n=>n.id===id)?.name||'All spaces';}
function cssColor(name){return getComputedStyle(document.documentElement).getPropertyValue('--'+name).trim();}
function art(p){
 const begin=`<svg viewBox="0 0 320 200" aria-hidden="true" focusable="false" xmlns="http://www.w3.org/2000/svg"><rect width="320" height="200" fill="#282828"/>`;
 const end=`</svg><span class="art-label">${p.label}</span>`;
 let s='';
 if(p.art==='bird')s=`<circle cx="233" cy="43" r="82" fill="#384737"/><circle cx="235" cy="43" r="59" fill="none" stroke="#64784b"/><path d="M0 143 Q90 90 180 140 T340 104 V200 H0" fill="#2c3b30"/><path d="M34 148 L285 125 M129 141 L152 154" stroke="#9d9167" stroke-width="4"/><path d="M149 137 C123 131 122 101 146 93 C149 74 170 71 178 86 L193 92 L177 97 C175 123 165 137 149 137Z" fill="#ebdbb2"/><path d="M144 103 Q105 94 94 67 Q149 77 162 112" fill="#b8bb26"/><circle cx="167" cy="86" r="3" fill="#282828"/><path d="M152 136 L150 142 M160 134 L161 140" stroke="#ebdbb2" stroke-width="2"/><path d="M214 78 q15 12 0 24 M224 70 q23 20 0 40" fill="none" stroke="#8ec07c" stroke-width="2"/>`;
 if(p.art==='synth')s=`<rect x="28" y="29" width="264" height="129" rx="7" fill="#504945" stroke="#65604d"/>${[0,1,2,3,4,5].map((v)=>`<rect x="${39+v*41}" y="39" width="34" height="108" rx="2" fill="${v%2?'#3c3836':'#665c54'}"/><circle cx="${56+v*41}" cy="60" r="9" fill="#1d2021" stroke="#bdae93"/><path d="M${56+v*41} 60v-7" stroke="#fabd2f"/><circle cx="${56+v*41}" cy="106" r="4" fill="#fe8019"/>`).join('')}<path d="M56 106 C60 178 216 169 220 106 M97 106 C130 48 272 168 261 106 M138 106 C135 153 183 154 179 106" fill="none" stroke="#d3869b" stroke-width="3"/><path d="M96 106 Q140 171 219 106" fill="none" stroke="#fabd2f" stroke-width="3"/>`;
 if(p.art==='garden')s=`<rect width="320" height="200" fill="#334035"/><circle cx="267" cy="39" r="19" fill="#d6b971"/><path d="M-30 171 L133 65 L334 167 L175 242Z" fill="#75834f"/><path d="M1 170 L135 85 L301 167 L172 222Z" fill="#a6a65c"/><path d="M72 193 L149 143 L214 174 L243 155" fill="none" stroke="#d9c28c" stroke-width="18"/>${[[81,135],[119,109],[205,139],[257,163],[151,177]].map(([x,y])=>`<rect x="${x-2}" y="${y-15}" width="5" height="28" fill="#655c3e"/><path d="M${x-20} ${y} l20 -40 l20 40Z" fill="#496847"/><path d="M${x-15} ${y-15} l15 -30 l15 30Z" fill="#678352"/>`).join('')}<path d="M152 126 l27 -17 l28 17 v23 l-28 17 l-27 -17Z" fill="#c5a56b"/><path d="M147 127 l31 -29 l35 28 l-35 19Z" fill="#b5653e"/>`;
 if(p.art==='terminal')s=`<rect x="27" y="25" width="266" height="137" rx="5" fill="#1d2021" stroke="#526354"/><path d="M27 47H293" stroke="#526354"/><circle cx="40" cy="36" r="3" fill="#fb4934"/><circle cx="51" cy="36" r="3" fill="#fabd2f"/><circle cx="62" cy="36" r="3" fill="#b8bb26"/><g fill="#8ec07c" font-family="monospace" font-size="10"><text x="42" y="68">~/thoughts $ find a connection</text><text x="42" y="91" fill="#ebdbb2">↳ something you almost forgot</text><text x="42" y="112" fill="#a89984">  3 related fragments found</text><text x="42" y="140">&gt; _</text></g><path d="M224 97 l15 -17 l28 26 l-19 23Z" fill="none" stroke="#b8bb26"/><circle cx="224" cy="97" r="4" fill="#b8bb26"/>`;
 if(p.art==='sensor')s=`<rect width="320" height="200" fill="#343c31"/><path d="M39 200 Q22 88 71 22 M281 200 Q290 100 247 24" fill="none" stroke="#798856" stroke-width="2"/>${[40,75,110].map(y=>`<ellipse cx="54" cy="${y}" rx="9" ry="25" fill="#596b43" transform="rotate(-35 54 ${y})"/>`).join('')}<rect x="99" y="38" width="124" height="115" rx="13" fill="#d5c79e"/><rect x="111" y="50" width="100" height="70" rx="4" fill="#444d39"/><text x="122" y="81" fill="#cbd69a" font-size="24" font-family="monospace">24.6°</text><text x="122" y="104" fill="#cbd69a" font-size="9" font-family="monospace">SOIL 68% / GOOD</text><circle cx="162" cy="137" r="6" fill="#706d50"/>`;
 if(p.art==='vision')s=`<rect width="320" height="200" fill="#3c3836"/>${Array.from({length:15},(_,i)=>`<ellipse cx="${75+i*12}" cy="${88+Math.sin(i*.6)*23}" rx="${25+i*2}" ry="${65-i*2}" fill="none" stroke="${['#d3869b','#fe8019','#fabd2f','#83a598'][i%4]}" opacity=".7" transform="rotate(${i*8} ${75+i*12} 100)"/>`).join('')}<path d="M22 47V25H45 M277 25h22v22 M299 144v22h-22 M45 166H22v-22" fill="none" stroke="#ebdbb2" stroke-width="1"/>`;
 if(p.art==='trace')s=`<rect width="320" height="200" fill="#282828"/>${[0,1,2,3,4].map(i=>`<path d="M30 ${50+i*22}H290" stroke="#3c4a43"/>`).join('')}<path d="M35 116H75V63H121V95H166V43H212V119H279" fill="none" stroke="#83a598" stroke-width="3"/>${[[75,63],[121,95],[166,43],[212,119]].map(([x,y])=>`<circle cx="${x}" cy="${y}" r="6" fill="#282828" stroke="#b8bb26" stroke-width="2"/>`).join('')}<text x="34" y="155" font-family="monospace" font-size="9" fill="#a89984">request_084 → 4 spans → 23ms</text>`;
 if(p.art==='timer')s=`<rect width="320" height="200" fill="#3c3836"/><circle cx="160" cy="88" r="58" fill="none" stroke="#5e5845"/><circle cx="160" cy="88" r="58" fill="none" stroke="#d5b574" stroke-width="3" stroke-dasharray="260 365" transform="rotate(-90 160 88)"/><text x="160" y="95" text-anchor="middle" font-family="monospace" font-size="28" fill="#ebdbb2">24:59</text><text x="160" y="117" text-anchor="middle" font-family="monospace" font-size="8" letter-spacing="2" fill="#bdae93">ROOM TO THINK</text>`;
 if(p.art==='energy')s=`<rect width="320" height="200" fill="#3c3836"/>${[36,50,40,69,80,55,43,63,97,70,51,32].map((h,i)=>`<rect x="${35+i*21}" y="${150-h}" width="12" height="${h}" rx="2" fill="${i===8?'#fabd2f':'#777c3a'}"/>`).join('')}<text x="35" y="36" font-family="monospace" font-size="12" fill="#ebdbb2">a quieter kind of power.</text><path d="M32 153H293" stroke="#bdae93"/>`;
 return begin+s+end;
}
function filtered(){let p=[...projects];if(state.mode==='saved')p=p.filter(x=>state.saved.has(x.id));else if(!state.selected&&!state.query.trim())p=p.filter(recent);if(state.selected){const ids=new Set(members(state.selected).map(x=>x.id));p=p.filter(x=>ids.has(x.id));}if(state.tech.size)p=p.filter(x=>x.tags.some(t=>state.tech.has(t)));const q=state.query.trim().toLowerCase();if(q)p=p.filter(x=>[x.name,x.desc,x.detail,...x.tags,...x.spaces.map(label)].join(' ').toLowerCase().includes(q));return p.sort((a,b)=>b.date.localeCompare(a.date));}
...
function renderIndex(){$('#domain-index').innerHTML=`<button class="domain-nav ${!state.selected?'active':''}" data-domain="">All domains <span>04</span></button>`+domains.map(d=>`<button class="domain-nav ${state.selected===d.id?'active':''}" data-domain="${d.id}">${d.name} <span>${String(count(d.id)).padStart(2,'0')}</span></button>`).join('');$('#domain-index').querySelectorAll('button').forEach(b=>b.onclick=()=>{if(!b.dataset.domain){state.selected=null;renderGraph(false);renderCards();renderIndex();}else setSelected(b.dataset.domain);});}
let positions=new Map();
function renderGraph(animate=true){
 cancelAnimationFrame(frame);
 const mobile=matchMedia('(max-width:750px)').matches;
 $('#graph').setAttribute('viewBox',mobile?'0 0 390 420':'0 0 1050 420');
 let visible=allNodes.filter(n=>(!n.child||state.expanded||(mobile&&n.parents?.includes(state.selected)))&&(!n.old||state.older));
 if(mobile){
   if(!state.selected)visible=visible.filter(n=>n.domain);
   else {const selected=allNodes.find(n=>n.id===state.selected);visible=visible.filter(n=>n.id===state.selected||n.parents?.includes(state.selected)||selected?.parents?.includes(n.id));}
 }
 const targets=new Map(visible.map((n,i)=>[n.id,mobile?(state.selected?(n.id===state.selected?{x:195,y:205}:{x:195+130*Math.cos(i*2*Math.PI/visible.length-Math.PI/2),y:210+130*Math.sin(i*2*Math.PI/visible.length-Math.PI/2)}):{x:105+(i%2)*180,y:135+Math.floor(i/2)*160}):{x:n.x,y:n.y}]));
 const ids=new Set(visible.map(n=>n.id));
 const edges=visible.flatMap(n=>(n.parents||[]).filter(p=>ids.has(p)).map(p=>({from:p,to:n.id})));
 let connected=state.selected?new Set([...descendantIds(state.selected),...(allNodes.find(n=>n.id===state.selected)?.parents||[])]):null;
 if(state.query.trim()||state.tech.size){const matches=new Set(filtered().map(p=>p.id));connected=new Set(allNodes.filter(n=>members(n.id).some(p=>matches.has(p.id))).map(n=>n.id));}
 $('#edges').innerHTML=edges.map(e=>`<line class="edge ${connected?.has(e.from)&&connected?.has(e.to)?'selected':''}" data-from="${e.from}" data-to="${e.to}"/>`).join('');
 $('#nodes').innerHTML=visible.map(n=>{const r=(n.domain?13:5)+Math.sqrt(count(n.id))*3;return `<g class="node ${n.domain?'domain':''} ${state.selected===n.id?'selected':''} ${connected&&!connected.has(n.id)?'dim':''}" data-id="${n.id}" role="button" tabindex="0" aria-pressed="${state.selected===n.id}" aria-label="${n.name}, ${count(n.id)} recent projects. Select to filter."><circle r="${r}" fill="${cssColor(n.color)}"/>${state.selected===n.id?`<circle class="ring" r="${r+7}"/>`:''}<text text-anchor="middle" y="${r+20}">${n.name}</text>${n.domain?`<text text-anchor="middle" y="${r+35}" style="font-size:9px;font-weight:400;fill:var(--muted)">${count(n.id)} recent / ${members(n.id).length} total</text>`:''}</g>`;}).join('');
 const nodeElements=[...$('#nodes').children],edgeElements=[...$('#edges').children];
 nodeElements.forEach(el=>{el.onclick=()=>{if(!moved)setSelected(el.dataset.id);};el.onkeydown=e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();setSelected(el.dataset.id);}};});
 for(const n of visible){const target=targets.get(n.id);if(mobile||!positions.has(n.id))positions.set(n.id,{x:target.x+(animate?Math.sin(n.x)*10:0),y:target.y+(animate?Math.cos(n.y)*10:0),vx:0,vy:0});}
 function draw(){nodeElements.forEach(el=>{const p=positions.get(el.dataset.id);el.setAttribute('transform',`translate(${p.x},${p.y})`);});edgeElements.forEach(el=>{const a=positions.get(el.dataset.from),b=positions.get(el.dataset.to);el.setAttribute('x1',a.x);el.setAttribute('y1',a.y);el.setAttribute('x2',b.x);el.setAttribute('y2',b.y);});}
 let tick=0;
 function settle(){for(const n of visible){const p=positions.get(n.id);const target=targets.get(n.id);p.vx+=(target.x-p.x)*.018;p.vy+=(target.y-p.y)*.018;for(const other of visible){if(other===n)continue;const b=positions.get(other.id);let dx=p.x-b.x,dy=p.y-b.y;const dist=Math.max(1,Math.hypot(dx,dy));if(dist<110){p.vx+=dx/dist*(110-dist)*.005;p.vy+=dy/dist*(110-dist)*.005;}}p.vx*=.78;p.vy*=.78;p.x+=p.vx;p.y+=p.vy;}draw();if(++tick<100)frame=requestAnimationFrame(settle);}
 if(animate&&!matchMedia('(prefers-reduced-motion: reduce)').matches)settle();else draw();
 $('#graph-caption').textContent=state.selected?label(state.selected).toUpperCase():'EVERY DOT IS A DIRECTION';
 $('#graph-hint').textContent=state.selected?`${count(state.selected)} recent · ${members(state.selected).length} total projects`:'Select to explore · drag to pan';
 applyTransform();
}
function applyTransform(){const cx=matchMedia('(max-width:750px)').matches?195:525;$('#viewport').setAttribute('transform',`translate(${state.panX} ${state.panY}) translate(${cx} 210) scale(${state.zoom}) translate(${-cx} -210)`);renderState();}
function zoom(change){state.zoom=Math.max(.6,Math.min(2.5,state.zoom+change));applyTransform();}
$('#zoom-in').onclick=()=>zoom(.2);$('#zoom-out').onclick=()=>zoom(-.2);$('#zoom-fit').onclick=()=>{state.zoom=1;state.panX=0;state.panY=0;applyTransform();};
$('#graph').addEventListener('pointerdown',e=>{moved=false;if(e.target.closest('.node'))return;const matrix=$('#graph').getScreenCTM();drag={x:e.clientX,y:e.clientY,px:state.panX,py:state.panY,scale:matrix?.a||1};$('#graph').setPointerCapture(e.pointerId);});
$('#graph').addEventListener('pointermove',e=>{if(!drag)return;const dx=e.clientX-drag.x,dy=e.clientY-drag.y;if(Math.abs(dx)+Math.abs(dy)>4)moved=true;state.panX=drag.px+dx/drag.scale;state.panY=drag.py+dy/drag.scale;applyTransform();});
...
function reset(){state.selected=null;state.query='';state.tech.clear();state.mode='explore';$('#search').value='';renderFilters();renderCards();renderGraph(false);renderIndex();document.querySelectorAll('[data-mode]').forEach(b=>b.classList.toggle('active',b.dataset.mode==='explore'));}
...
function setMode(mode){state.mode=mode;state.selected=null;state.query='';state.tech.clear();$('#search').value='';document.querySelectorAll('[data-mode]').forEach(b=>b.classList.toggle('active',b.dataset.mode===mode));if(mode==='saved'){state.variant='C';setVariant();}renderCards();renderFilters();renderGraph(false);renderIndex();}
...
function renderState(){
 const el=$('#switcher-state');if(el)el.textContent=`${label(state.selected)} · ${state.theme} · ${state.saved.size} saved`;
 const result=filtered();
 $('#focus-title').textContent=state.selected?label(state.selected):state.query?'Follow the matches.':'Follow your curiosity.';
 $('#focus-copy').textContent=state.selected?`${count(state.selected)} recent / ${members(state.selected).length} total · ${result.length} matching projects`:`4 domains · ${result.length} matching projects · fictional sample`;
...
function snapshot(){return {...state,tech:[...state.tech],saved:[...state.saved],visibleProjects:filtered().map(p=>p.id),data:'fictional',search:'keyword-only'};}
function PrototypeSwitcher(){if(window.PROTOTYPE_DEV===false)return;$('#prototype-switcher').innerHTML=`<aside class="switcher" aria-label="Prototype layout variations"><button id="variant-prev" aria-label="Previous layout">←</button><div class="switcher-center"><b><span class="variant-key" id="variant-label"></span></b><small id="variant-description"></small></div><button id="variant-next" aria-label="Next layout">→</button><span class="switcher-state" id="switcher-state"></span></aside>`;$('#variant-prev').onclick=()=>cycle(-1);$('#variant-next').onclick=()=>cycle(1);}
function cycle(step){const keys=Object.keys(names);state.variant=keys[(keys.indexOf(state.variant)+step+keys.length)%keys.length];setVariant();}
function setVariant(){
 ...
 if($('#variant-label')){$('#variant-label').textContent=names[state.variant];$('#variant-description').textContent=descriptions[state.variant];}
...
document.addEventListener('keydown',e=>{
 ...
 if(window.PROTOTYPE_DEV!==false&&(e.key==='ArrowLeft'||e.key==='ArrowRight')){e.preventDefault();cycle(e.key==='ArrowLeft'?-1:1);}
});
PrototypeSwitcher();setMobile('graph');renderFilters();renderIndex();renderCards();renderGraph();setVariant();
